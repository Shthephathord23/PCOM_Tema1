...
void send_arp_req(uint32_t dest_ip, size_t interface);
...
void send_arp_req(uint32_t dest_ip, size_t interface)
{
	...
	init_arp_hdr(&arp_req, ARP_REQUEST, src_mac, reinterpret_cast<uint32_t>(inet_addr(get_interface_ip(interface))), dest_ip);

	printf("Trimitem ARP request pentru %s\n", my_inet_ntoa(dest_ip));

	reinterpret_cast<ethernet_frame* >(&arp_req)->send_to_broadcast(ETHER_TYPE_ARP, src_mac, interface);
}
...
void send_arp_reply(arp_packet* arp_request)
{
	...
	init_arp_hdr(&arp_reply, ARP_REPLY, arp_table_map[dest_ip], dest_ip, *get_arp_src_ip(arp_request));
...
void handle_arp_packet(arp_packet* arp_pkt)
{
	...
	switch (opcode)
	{
		case ARP_REQUEST:
		{
			...
			if (dest_ip == reinterpret_cast<uint32_t>(inet_addr(get_interface_ip(*get_eth_interface(arp_pkt)))))
			{
				printf("Am gasit ARP request pentru mine\n");
		...
		case ARP_REPLY:
		{
			...
			if (waiting_packets.find(dest_ip) != waiting_packets.end())
			{
				...
				{
					ipv4_packet* packet_to_send = &waiting_packets[dest_ip].front();
...
void send_ipv4_packet(ipv4_packet* ipv4_packet, bool drop_if_no_route)
{
	...
	route_table_entry* best_route = lookup(trie_root, *get_ipv4_dest_ip(ipv4_packet));
	...
	if (arp_table_map.find(best_route->next_hop) == arp_table_map.end())
	{
		...
		{
			send_arp_req(best_route->next_hop, best_route->interface);
		}
		return;
	}
...
int main(int argc, char *argv[])
{
	...
	build_trie(trie_root, rtable, rtable_len);
